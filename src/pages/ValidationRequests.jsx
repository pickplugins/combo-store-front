...
function ValidationRequests() {
	...
	var [validateMailPrams, setvalidateMailPrams] = useState({ email: '', apikey: '', testType: "", result: {}, loading: false });
	...
	function fetchPosts() {
		...
		fetch(appData.serverUrl + "wp-json/combo-store/v2/validation_requests", {
			method: "POST",
			headers: {
				'Content-Type': 'application/json',
				'Authorization': `Bearer ${token}`
	...
	var columns = {
		id: { label: t("ID"), classes: "w-30" },
		email: { label: t("Email") },
		result: { label: t("Result"), classes: "w-60" },
		apikeyid: { label: t("API key"), linkTo: { path: 'apikeys' } },
